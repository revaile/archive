<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-10 flex gap-5 relative">
                <!-- KP Dropdown -->
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="bg-[#facc15] hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-lg">
                        + Create KP
                    </button>
                    <div x-show="open" @click.away="open = false" class="absolute mt-2 w-36 bg-white shadow-lg rounded-lg overflow-hidden z-50">
                        <a href="{{ route('dashboard.documents.create', ['category' => 'kp']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">KP</a>
                        <a href="{{ route('dashboard.documents.create', ['category' => 'mbkm']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">MBKM</a>
                        <a href="{{ route('dashboard.documents.create', ['category' => 'magang']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Magang</a>
                    </div>
                </div>
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" class="absolute mt-2 w-44 bg-white shadow-lg rounded-lg overflow-hidden z-50">
                        <a href="{{ route('dashboard.documents.create', ['category' => 'proposal']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Proposal</a>
                        <a href="{{ route('dashboard.documents.create', ['category' => 'proposal_bersama']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Proposal Bersama</a>
                    </div>
                </div>
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" class="absolute mt-2 w-40 bg-white shadow-lg rounded-lg overflow-hidden z-50">
                        <a href="{{ route('dashboard.documents.create', ['category' => 'skripsi']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Skripsi</a>
                        <a href="{{ route('dashboard.documents.create', ['category' => 'artikel']) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Artikel</a>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-between mb-7">
                <form method="GET" action="{{ route('dashboard.documents.index.search') }}"
                    ...
                    <select name="category"
                        ...
                        <option value="proposal_bersama" {{ request('category') == 'proposal' ? 'selected' : '' }}>Proposal_bersama
                        </option>
